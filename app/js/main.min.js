...
// ==============================================================
// WebP
// Скрипт нужен для плагина gulp-webp-html и webpcss.
// Скрипт проверяет, поддерживает ли браузер формат webp, если да,
// то добавляет класс _webp тегу body.
// Тогда плагин gulp-webp-html, видя этот класс, сможет динамически подставлять тег picture в структуру документа
// А плагин webpcss, видя этот класс, сможет вставлять в css изображения .webp по селектору класса ._webp
...
// ==============================================================
// DigiFormat
// Разбивает переданное число (1000000 разобьет на 1 000 000)
...
function digi_animate_value(el, start, end, duration) {
  var obj = el;
  var range = end - start;
  // Таймер не более 50 мс (not really visible any way)
  var minTimer = 50;
  // время шага CALL, чтобы показать все промежуточные значения
  var stepTime = Math.abs(Math.floor(duration / range));

  // Никогда не ходите ниже минимального таймера
  stepTime = Math.max(stepTime, minTimer);

  // получить текущее время и рассчитать желаемое время окончания
...
//   document.querySelector('.popup_' + modal).classList.remove('_active');
...
for (let index = 0; index < popup_link.length; index++) {
  ...
  el.addEventListener('click', function (e) {
    if (unlock) {
      ...
      //   removeModal(el.classList.contains('md-setperspective'), item);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  openStreamBlockBtn.addEventListener('click', function (e) {
    ...
    console.log(streamsBody.offsetWidth);
  });
...
if (document.querySelector('.slider-info__body')) {
  let mainSlider = new Swiper('.slider-info__body', {
    ...
    //-------------------
    // Lasy Loading
    // - Настройки следует включать тогда, когда slidesPerView: 'auto' или больше 1
    // Отключить предзагрузку картинок, по умолч. true
    preloadImages: false,
    // lazy: true,
    //-------------------
    // Включаем параллакс
...
// // require('./other_scripts.js'); // Require Other Script(s) from app/js folder Example
// // import '~/app/libs/mmenu/dist/mmenu.js';

// // Swiper
// // core version + navigation, pagination modules:
// import Swiper, { Navigation, Pagination, Parallax, Mousewheel, Scrollbar, Controller } from 'swiper';
// // configure Swiper to use modules
// Swiper.use([Navigation, Pagination, Parallax, Mousewheel, Scrollbar, Controller]);
